...
class LearningService
{
    public function refresh(Enrollment $enrollment): void
    {
        DB::transaction(function () use ($enrollment) {
            ...
            foreach ($course->quizzes()->where('required', true)->get() as $quiz) {
                $ready = $ready && $quiz->attempts()->where('user_id', $e->user_id)->where('passed', true)->exists();
            }
            foreach ($course->assignments()->where('required', true)->get() as $assignment) {
                $ready = $ready && $assignment->submissions()->where('user_id', $e->user_id)->where('status', 'graded')->where('grade', '>=', 70)->exists();
            }
            if (! $ready) {
                return;
            }
